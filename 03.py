...
import requests
from aoc import get_ints, build_number
...
class Solution:
    ...
    def optimize(self, n):
        ...
        for row in self.data:
            value = []
            left, right = -1, len(row) - n
            while left < right and right < len(row):
                # find left most new position (larger than current left)
                max_possible_position = right
                for p in range(right - 1, left, -1):
                    if row[p] >= row[max_possible_position]:
                        max_possible_position = p
                if right == max_possible_position:
                    # nothing changed -> nothing will change anymore
                    break
                # take the best digit at max_possible_position instead of current digit in row (row[right])
                value.append(row[max_possible_position])
                left = max_possible_position
                right += 1
            number = build_number(value, row[right:])
            print(value, row[right:], number)
            result += number

        return result

    def first_part(self):
        return self.optimize(2)

    def second_part(self):
        return self.optimize(12)
